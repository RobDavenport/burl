...
## Objective
Implement append-only event logging under `.fdx/.workflow/events/` to support audit/recovery across machines.

## Context
Source of truth: `fdx.md` section “Logging & Observability”.

## Requirements
- Implement an event record type that can serialize to a single-line JSON object.
- Append events to `.fdx/.workflow/events/events.ndjson` (create file if missing).
- Ensure:
...
- Update commands implemented earlier to emit required events:
  - `fdx init` must append an `init` event after scaffolding is created (and commit/push workflow branch per config).
  - `fdx lock clear` must append a `lock_clear` event including which lock was cleared, its age, and whether `--force` was used.
- Ensure event writes follow the PRD’s transactional guidance:
  ...
  - for `fdx lock clear` specifically: it must be able to clear `workflow.lock`, so logging/committing should be best-effort if `workflow.lock` cannot be acquired until after the lock is cleared.
