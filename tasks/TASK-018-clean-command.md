---
id: TASK-018
title: Implement `burl clean` (completed/orphan worktrees cleanup; safe-by-default)
priority: medium
depends_on: [TASK-007, TASK-008, TASK-015, TASK-017]
---

## Objective
Implement `burl clean` to remove local artifacts safely:
- completed task worktrees
- orphan worktree directories

## Context
Source of truth: `burl.md` “clean” command note and “Security & Safety Considerations”.
...
## Acceptance Criteria
- [ ] `burl clean` (no flags) performs no deletions and prints a plan.
- [ ] `burl clean --force` removes only directories under `.worktrees/`.
- [ ] `burl clean` never deletes branches by default.
...
- Run `burl clean` and assert it reports the dir.
- Run `burl clean --force` and assert the dir is deleted.
