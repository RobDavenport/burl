...
- All commands must treat the canonical workflow worktree as the single source of truth:
  - workflow worktree: default `.fdx/`
  - workflow state dir: default `.fdx/.workflow/`
...
- Provide a helper that resolves:
  ...
  - and returns a user error (exit `1`) with “run `fdx init`” when the workflow worktree/state dir is missing (except for `fdx init` itself).
...
- In V1, treat the workflow layout as **fixed defaults**:
  ...
  - workflow branch: `fdx`
  - workflow state dir: `.fdx/.workflow/`
...
### Config model
Implement a `Config` representation for `.fdx/.workflow/config.yaml` with at least:
...
- For commands other than `init`, missing config is a user error (exit `1`) with a fix: “run `fdx init`”.
...
## Acceptance Criteria
- [ ] Running `fdx` inside a non-git directory prints a clear error and exits `1`.
- [ ] Repo root resolution works from nested worktrees (unit/integration test).
- [ ] Workflow state resolution always targets `.fdx/.workflow/` even when run inside a task worktree.
